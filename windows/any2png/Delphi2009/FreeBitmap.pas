...
type
  ...
  private
    ...
    function getDPI_H: Double;
    function getDPI_Y: Double;
    function getFIH_MM: Double;
    function getFIW_MM: Double;
  protected
  ...
  public
    ...
    property ImageHeightMM:Double read getFIH_MM;
    property ImageWidthMM:Double read getFIW_MM;
    property DPI_H:Double read getDPI_H;
    property DPI_Y:Double read getDPI_Y;
  end;
...
begin
  if FDib <> nil then
  begin
    dib_ := FreeImage_ConvertToType(FDib, ImageType, ScaleLinear);
    Result := Replace(dib_)
  end
  else
    Result := False
...
begin
  if FDib <> nil then
  begin
    dib_ := FreeImage_Dither(FDib, Algorithm);
    Result := Replace(dib_);
  end
  else
    Result := False;
...
function TFreeBitmap.getDPI_H: Double;
begin
  Result :=2.54*GetHorizontalResolution;
end;

function TFreeBitmap.getDPI_Y: Double;
begin
  Result :=2.54*GetVerticalResolution;
end;

function TFreeBitmap.getFIH_MM: Double;
begin
 if isValid then
  Result :=10*GetHeight/GetVerticalResolution
   Else
    Result :=0;
end;
...
function TFreeBitmap.getFIW_MM: Double;
begin
 if isValid then
  Result :=10*GetWidth/GetHorizontalResolution
   Else
    Result :=0;
end;
